...
class RecipeViewSet(ModelViewSet):
    ...
    def download_shopping_cart(self, request):
        recipes = IngredientRecipe.objects.filter(recipe__in=request.user.shopping_cart.all())
        ...
        recipes_name = ', '.join(set([recipe.recipe.name for recipe in recipes]))
...
class APIFollow(APIView):
    ...
    def post(self, request, pk):
        ...
        if follow_serializer.is_valid():
            ...
            serializer = FollowUsersSerializer(user, context={'request': request})
            return Response(serializer.data, status=status.HTTP_201_CREATED)
...
class FollowsViewSet(ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        return User.objects.filter(following__following_user_id=self.request.user.id)
